...
7 GOSUB HELP 
10 R =511 S=1 N=0 P=0 ? R ;
20 LOOP ' PWM loop 
22 IF K=P N=N+1 S=N/10+1
24 IF K<>P S=1 N=0
26 P=K K=0 
...
70 IF KEY?  K=KEY GOSUB UPPER 
72 IF (K=ASC(\D) OR K=ASC(\U)) AND K=P N=N+1 S=N/10+1
74 IF K=0 OR K<>P S=1 N=0
78 IF K=0 GOTO 30 
80 IF K =ASC (\U) GOTO 200 
84 IF K =ASC (\F)  R =1023 GOTO 600 REM  pleine intensite 
90 IF K =ASC (\D) GOTO 400 
94 IF K =ASC (\O) R =0 GOTO 600 REM  eteindre
96 IF K= ASC(\?) GOSUB HELP GOTO 600
100 IF K =ASC (\Q) GOSUB CLS END 
110 GOTO LOOP
200 IF R <1023 R =R +S GOTO 600 
210 GOTO LOOP
400 IF R >0  R =R -S GOTO 600 
410 GOTO LOOP
600 IF R<0 R=0 
602 IF R>1023 R=1023 
